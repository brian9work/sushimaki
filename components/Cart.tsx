...
export default function Cart() {
  ...
  return (
    ...
    '>
      ...
      <div className='bg-white rounded-3xl w-11/12 max-w-md shadow-2xl z-10 flex flex-col overflow-hidden animate-slide-in max-h-[90vh]'>
        {/* Header */}
        <div className="p-5 border-b border-gray-100 flex justify-between items-center bg-white sticky top-0 z-10 shrink-0">
          <div>
            <h2 className='font-bold text-2xl text-gray-800'>Tu Carrito</h2>
            <p className="text-gray-500 text-sm">{cartList.length} {cartList.length === 1 ? 'item' : 'items'}</p>
          </div>
          <button
            onClick={handleClose}
